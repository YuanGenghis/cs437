...
parser.add_argument("-n", "--thingName", action="store", dest="thingName", default="vehicle2", help="Targeted thing name")
...
if args.mode == 'both' or args.mode == 'subscribe':
    myAWSIoTMQTTClient.subscribe("max/co2/3", 0, None)
time.sleep(2)

vehicle_data = json.loads(open('./vehicle2.json').read())
vehicle_co2_json = vehicle_data['vehicle_CO2']
vehicle_id_json = vehicle_data['vehicle_id']

loopCount = 0
while True:
    if args.mode == 'both' or args.mode == 'publish':
        if loopCount == 0:
            for i in range(len(vehicle_co2_json)):
                vehicle_json = {
                "vehicle_CO2": vehicle_co2_json[str(i)],
                "vehicle_id": vehicle_id_json[str(i)]
                }
                messageJson = json.dumps(vehicle_json)
                myAWSIoTMQTTClient.publish("max/co2/2/trigger", messageJson, 0)
